...
case class UploadProgress(chunksInS3: Int, chunksInYouTube: Int, fullyUploaded: Boolean, fullyTranscoded: Boolean,
                          retries: Int, copyProgress: Option[CopyProgress] = None)

case class CopyETag(number: Int, eTag: String)
case class CopyProgress(copyId: String, fullyCopied: Boolean, eTags: List[CopyETag])
...
object CopyProgress {
  implicit val format: Format[CopyProgress] = Jsonx.formatCaseClass[CopyProgress]
}
