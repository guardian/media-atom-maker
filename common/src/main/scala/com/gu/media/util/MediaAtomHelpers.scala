...
object MediaAtomHelpers {
  ...
  /**
   * Tries to create a meaningful User object by inferring a user's name based on an email address, specifically a
   * guardian email, for saving changes to atoms where no name is explicitly available.
   *
   * Covers common patterns such as joe.bloggs@guardian.co.uk, joe.bloggs.contractor@guardian.co.uk, but falls back to
   * blank names if no match found.
   *
   * @param email
   * @return
   */
  ...
  /**
   * Takes a SelfHostedAsset that has sources as relative s3 keys and rewrites the sources as full urls.
   *
   * Where forward slashes are detected in the relative key, the key is split into path and filename and
   * only the filename is url encoded, so that the slashes are preserved.  This is to satisfy the m3u8 transcoder,
   * which uses relative paths in its manifest.
   * e.g.
   ...
   *
   * @param selfHostedAsset - self-hosted asset with sources like "some/path/some-file"
   * @param selfHostedOrigin - the prefix for the urls e.g. https://gu.com/videos
   * @return the self-hosted asset with url-encoded sources
   */
  def urlEncodeSources(selfHostedAsset: SelfHostedAsset, selfHostedOrigin: String): SelfHostedAsset = {
    ...
    case SelfHostedAsset(sources) =>
      ...
      val subtitleAssets = sources.collect {
        case VideoSource(src, VideoSource.mimeTypeM3u8) =>
          ...
          ThriftAsset(AssetType.Subtitles, version, subtitleSrc, ThriftPlatform.Url, Some(VideoSource.mimeTypeVtt))
      }
