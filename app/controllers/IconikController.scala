...
//
//      (MediaAtomHelpers.getCurrentAssetVersion(atom), atom.iconikData) match {
//        case (None, _) =>
//          log.warn(
...
//        case (_, None) =>
//          log.warn(
//            "Requested re-index on an item with no iconik data, returning bad request"
//          )
//          BadRequest(
//            Json.toJson(
//              Map(
//                "status" -> "bad_data",
//                "detail" -> s"$id does not have iconik data attached"
//              )
//            )
//          )
//        case (Some(currentAssetVersion), Some(iconikData)) =>
//          val versionWithId = id + s"-$currentAssetVersion"
//          val iconik = new IconikUploadActions(awsConfig)
//          iconik.sendToIconik(
//            IconikResyncMetadataMessage.build(
//              versionWithId,
//              atom,
//              awsConfig
//            )
