...
case class DeleteAssetListCommand(
  ...
  def process(): T = {
    ...
    val assetsToDelete: Seq[Asset] = mediaAtom.assets.filter(asset => assets.exists(_.id == asset.id))
    ...
    if (assetsToDelete.nonEmpty) {
      ...
      val updatedAtom = mediaAtom.copy(
        assets = mediaAtom.assets.filterNot(asset => assets.exists(_.id == asset.id))
      )

      UpdateAtomCommand(atomId, updatedAtom, stores, user, awsConfig).process()
    } else {
      log.warn(markers, s"Asset version ${assetsVersion} does not exist on atom $atomId so cannot be deleted")
      AssetNotFound
    }
  }
}
