...
case class CommandException(msg: String, responseCode: Int)
    extends RuntimeException(msg)

object CommandExceptions extends Results {

  def UnknownFailure =
    throw new CommandException("Unknown internal server error", 500)
  ...
  def YouTubeConnectionIssue =
    throw new CommandException("Could not connect to YouTube", 500)
  def NotYoutubeAsset =
  ...
  def AssetParseFailed =
    throw new CommandException("Failed to parse asset", 400)
  def AssetNotFound = throw new CommandException("Asset not found", 404)
  def CannotDeleteActiveAsset =
    throw new CommandException("Cannot delete active asset", 400)

  def AssetNotFound(assetId: String) =
    throw new CommandException(s"Asset with id $assetId not found", 404)
  def YoutubeException(err: String) = throw new CommandException(
    s"Exception when trying to reach YouTube: $err",
    400
  )
  def AtomUpdateFailed(err: String) =
    throw new CommandException(s"Failed to update atom: $err", 500)
  def AtomUpdateConflictError(err: String) =
    throw new CommandException(s"Failed to update atom: $err", 409)
  def AtomPublishFailed(err: String) =
    throw new CommandException(s"Failed to publish atom: $err", 500)

  def AtomMissingYouTubeChannel =
    throw new CommandException("Atom is missing YouTube channel", 400)
  def YouTubeVideoDoesNotExist(id: String) =
    throw new CommandException(s"YouTube video $id does not exist", 400)
  def IncorrectYouTubeChannel = throw new CommandException(
    s"New video is not on the same YouTube channel",
    400
  )
  def NotGLabsAtom = throw new CommandException(
    s"Third party videos can only be added to GLabs Atoms",
    400
  )

  def SubtitleFileUploadFailed =
    throw new CommandException("Failed to upload subtitle file", 500)
  ...
  def commandExceptionAsResult: PartialFunction[Throwable, Result] = {
    ...
    case YouTubeError(msg, false) =>
      InternalServerError(msg).withHeaders("X-No-Alerts" -> "true")
  }
}
...
object YouTubeError extends Logging {
  ...
  private def buildYouTubeException(
      code: Int,
      domain: String,
      message: String,
      reason: String
    ...
    val builder =
      new HttpResponseException.Builder(503, null, new HttpHeaders())
