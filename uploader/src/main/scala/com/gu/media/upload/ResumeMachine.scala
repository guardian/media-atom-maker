...
import software.amazon.awssdk.services.sfn.model.{GetExecutionHistoryRequest, HistoryEventType, SendTaskSuccessRequest}
...
class ResumeMachine
  ...
  override def handle(data: MediaConvertEvent): String = {
    ...
    val historyResults = stepFunctionsClient.getExecutionHistory(historyRequest).events().asScala
    val lastScheduledEvent = historyResults.find(_.`type` == HistoryEventType.TASK_SCHEDULED).head
