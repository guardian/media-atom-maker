import { getAspectRatioFromImage, getAspectRatioFromVideo, isImageCropOutOfSync } from '../util/getAspectRatio';
import { audioAsset1, emptyImage, emptyVideo, imageWithAspectRatio5by3, imageWithAspectRatio5by4, imageWithDefaultVideoAspectRatio, videoAsset1, videoAsset2, videoWithActiveAsset5by4 } from './models';
...
describe('isImageCropOutOfSync', () => {
    ...
    it('should return false if no video active image is set and the image is set to the default video value', () => {
        expect(isImageCropOutOfSync({...emptyVideo, posterImage: imageWithDefaultVideoAspectRatio})).toBe(false);
    });
     it('should return true if no video active image is set and the image is not set to the default video value', () => {
        expect(isImageCropOutOfSync({...emptyVideo, posterImage: imageWithAspectRatio5by3 } )).toBe(true);
    });
    it('should return false if a video active image is set and the image is set to the same value', () => {
        expect(isImageCropOutOfSync({...videoWithActiveAsset5by4, posterImage: imageWithAspectRatio5by4} )).toBe(false);
    });
     it('should return true if a video active image is set and the image is not set to the same value', () => {
        expect(isImageCropOutOfSync({...videoWithActiveAsset5by4, posterImage: imageWithAspectRatio5by3})).toBe(true);
    });
});