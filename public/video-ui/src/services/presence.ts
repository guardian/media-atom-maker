...
declare global {
  interface Window {
    presenceClient?: {
      (
        endpoint: string,
        config: Omit<PresenceConfig, 'domain'>
      ): PresenceClient;
    };
  }
}
...
const safelyStartPresence = (
  ...
  if (!window.presenceClient) {
    console.error(
      'Failed to connect to Presence as client was not available in window.'
    );
    reportPresenceClientError(
      new Error('Presence client function not available in window')
    );
    return;
  }
