...
export default class GridImageSelect extends React.Component<Props, State> {
  ...
  onUpdatePosterImage = (cropData: { assets: { secureUrl: string; mimeType: string; size: number; dimensions: { width: number; height: number; }; }[]; master: { secureUrl: string; mimeType: string; size: number; dimensions: { width: number; height: number; }; }; specification: { aspectRatio: string; uri: string; }; }, imageData: { data: { metadata: { credit: string; }; }; }) => {
