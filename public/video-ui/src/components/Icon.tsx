...
function renderText(children?: React.ReactNode, textClass?: string) {
  if (!children) {
    return null;
  }

  return (
    <span className={`icon--text responsive--${textClass || 'optional'}`}>
      {children}
    </span>
  );
}
...
export default function Icon({
  ...
  return (
    <span className={`${responsiveClassName} ${disabled ? 'disabled' : ''}`}>
      <i className="icon responsive--primary" onClick={onClick}>
        {icon}
      </i>
      {renderText(children, textClass)}
    </span>
  );
}
