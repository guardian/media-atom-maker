...
export class ManagedForm extends React.Component<Props> {
  ...
  getFormClass = () => {
    if (
      React.Children.toArray((this.props as any).children).some(child => (child as any).type.componentType === 'managedSection')
    ) {
  ...
  render() {
    const hydratedChildren = React.Children.toArray((this.props as any).children)
    .filter(child => !!child)
    .map(child => 
