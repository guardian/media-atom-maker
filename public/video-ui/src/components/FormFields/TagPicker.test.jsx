  ...
  it('searches for tags as user types', async () => {
    mockedGetTagsByType.mockResolvedValue({
      data: [
        {
          data: {
            path: 'keyword/first-tag',
            externalName: 'Tag first external',
            internalName: 'Tag first (internal)'
          }
        },
        {
          data: {
            path: 'keyword/second-tag',
            externalName: 'Tag second external',
            internalName: 'Tag second (internal)'
          }
        }
      ]
    });
    ...
    expect(defaultProps.onUpdateField.mock.calls[0][0]).toEqual([
      'keyword/second-tag'
    ]);
  });
