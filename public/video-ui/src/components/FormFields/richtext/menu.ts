...
const wrapListItem =
  (schema: Schema) => (nodeType: NodeType, options: Partial<MenuItemSpec>) => {
    const passedOptions: Partial<MenuItemSpec> = {
      active: state => {
        return isInNode(state, nodeType);
      },
      ...options
    };
...
export const buildMenuItems = (schema: Schema) => {
  ...
  if (schema.marks.strong) {
    markMenu.push(
      markItem(schema.marks.strong, { title: 'Bold', label: 'format_bold' })
    );
  }
  if (schema.marks.em) {
    markMenu.push(
      markItem(schema.marks.em, { title: 'Italic', label: 'format_italic' })
    );
  }
  if (schema.marks.link) {
    markMenu.push(
      linkItem(schema.marks.link, { title: 'Link', label: 'link' })
    );
    markMenu.push(
      unlinkItem(schema.marks.link, { title: 'Unlink', label: 'link_off' })
    );
  }
  if (schema.nodes.bullet_list && schema.nodes.list_item) {
    markMenu.push(
      wrapListItem(schema)(schema.nodes.bullet_list, {
        title: 'Bullet list',
