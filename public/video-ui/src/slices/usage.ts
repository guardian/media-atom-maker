...
export const fetchUsages = createAsyncThunk<UsageData, string>(
  'usage/fetchUsages',
  (id, { dispatch }) => {
    return VideosApi.getVideoUsages(id).catch((error: unknown) => {
...
const usage = createSlice({
  ...
  reducers: {
    ...
    updateVideoUsageWebTitle(state, action: PayloadAction<string>) {
      const updateWebtitle = (usage: CapiContent): CapiContent => ({
        ...usage,
        webTitle: action.payload
      });
      state.data.preview.video = state.data.preview.video.map(updateWebtitle);
      state.data.published.video =
        state.data.published.video.map(updateWebtitle);
    },
  ...
  extraReducers: builder => {
    builder.addCase(fetchUsages.fulfilled, (state, { payload }) => {
      state.data = payload;

      const { preview, published } = payload;
      const totalVideoPages = preview.video.length + published.video.length;
      const totalUsages =
        totalVideoPages + preview.other.length + published.other.length;
