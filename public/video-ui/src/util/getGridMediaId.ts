...
export const getGridQueryParams = (
  cropType: string,
  video: Video,
  cropOptions: CropOption[]
) => {
  if (cropType === 'verticalVideo')
    return `cropType=${cropType}&customRatio=${cropType},9,16`;
  if (cropType === 'custom') {
    const aspectRatio = getAspectRatioFromVideo(video);
    if (!aspectRatio) return `cropType=video`;
    const existingGridCrop = cropOptions.find(
      c => c.ratioString === aspectRatio
    );
