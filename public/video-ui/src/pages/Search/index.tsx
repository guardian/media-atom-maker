...
const Videos = ({
  videos,
  total,
  search: { searchTerm, shouldUseCreatedDateForSort, videoPlayerFormatFilter },
  limit,
  presenceActions
}: VideosProps) => {
            ...
            const initialState = visitorData.data.subscribedTo
              .map(subscribedTo => {
                return {
                  ...
                  presences: subscribedTo.currentState
                } as VideoPresences;
              })
  ...
  const getPresencesForVideo = (videoId: string): Presence[] => {
    return videoPresences
      .filter(videoPresence => videoPresence.mediaId === `media-${videoId}`)
      .flatMap(videoPresence => videoPresence.presences);
  };
  ...
  useEffect(() => {
    if (presencesQueue) {
      setVideoPresences([
        ...videoPresences.filter(
          videoPresence =>
            videoPresence.mediaId !== presencesQueue.subscriptionId
        ),
        {
          ...
          <ul className="grid__list">
            {videos.map(video => (
              <ErrorBoundary key={video.id} fallback={<VideoItemError />}>
                <VideoItem
                  video={video}
                  presences={getPresencesForVideo(video.id)}
                />
              </ErrorBoundary>
            ))}
          </ul>
        ) : (
          <p className="grid__message">No videos found</p>
        )}
      </div>
      {videos.length === total ? null : (
        <MoreLink onClick={showMore}></MoreLink>
      )}
    </div>
  );
};
...
function mapStateToProps(state: {
  videos: { entries: number; total: number; limit: number };
  search: Search;
}) {
  return {
