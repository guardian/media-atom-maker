...
object Config {
  ...
  /* Test */
  lazy val targetBaseUrl = if (stage == "PROD") config.getString("targetBaseUrlProd") else config.getString("targetBaseUrlCode")
  ...
  /* Panda */
  lazy val domain = if (stage == "PROD") config.getString("domain-prod") else config.getString("domain-code")
