...
class ThriftUtilSpec extends AnyFunSpec
  ...
  describe("ThriftUtil") {
    ...
    it("should create empty assets without uri") {
      parseMediaAtom(Map.empty) should matchPattern { case Right(MediaAtom(Nil, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)) => }
    }

    it("should create multiple assets with multiple uri params") {
      inside(parseMediaAtom(Map("uri" -> List(youtubeUrl, youtubeUrl)))) {
        case Right(MediaAtom(assets, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)) =>
